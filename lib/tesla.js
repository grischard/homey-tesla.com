...
Tesla.prototype.controlChargePort = function (vehicleId, portstate) {
  var self = this
  return new Promise(function (resolve, reject) {
    if (typeof portstate !== 'boolean') return reject('state_invalid')
    checkActiveGrant(self).then(function () {
      if (portstate) {
        postVehicleCommand(self.grant.access_token, vehicleId, 'command/charge_port_door_open', null, function (error, state) {
          if (error) reject(error)
          else resolve(state)
        })
      } else {
        reject('closing_not_supported')
      }
    })
  })
}
...
Tesla.prototype.controlCharging = function (vehicleId, state) {
  ...
  return new Promise(function (resolve, reject) {
    if (typeof state !== 'boolean') return reject('state_invalid')
    checkActiveGrant(self).then(function () {
      postVehicleCommand(self.grant.access_token, vehicleId, `command/charge_${state ? 'start' : 'stop'}`, null, function (error, state) {
        if (error) reject(error)
        else resolve(state)
      })
    })
...
Tesla.prototype.controlDoorLock = function (vehicleId, state) {
  ...
  return new Promise(function (resolve, reject) {
    if (typeof state !== 'boolean') return reject('state_invalid')
    checkActiveGrant(self).then(function () {
      postVehicleCommand(self.grant.access_token, vehicleId, `command/door_${state ? 'lock' : 'unlock'}`, null, function (error, state) {
        if (error) reject(error)
        else resolve(state)
      })
    })
...
Tesla.prototype.controlValetMode = function (vehicleId, state, pin) {
  ...
  return new Promise(function (resolve, reject) {
    ...
    checkActiveGrant(self).then(function () {
      var data = {on: state}
      if (pin) {
        if (!(/^\d{4}$/.test(pin))) return reject('pin_not_4_digits')
        data.password = pin
      }
      postVehicleCommand(self.grant.access_token, vehicleId, 'command/set_valet_mode', data, function (error, state) {
        if (error) reject(error)
        else resolve(state)
      })
    })
...
Tesla.prototype.remoteStart = function (vehicleId) {
  var self = this
  return new Promise(function (resolve, reject) {
    if (!self.password) return reject('no_password')
    checkActiveGrant(self).then(function () {
      postVehicleCommand(self.grant.access_token, vehicleId, 'command/remote_start_drive', {password: self.password}, function (error, state) {
        if (error) reject(error)
        else resolve(state)
      })
    })
...
Tesla.prototype.resetValetPin = function (vehicleId) {
  ...
  return new Promise(function (resolve, reject) {
    checkActiveGrant(self).then(function () {
      postVehicleCommand(self.grant.access_token, vehicleId, 'command/reset_valet_pin', null, function (error, state) {
        if (error) reject(error)
        else resolve(state)
      })
    })
...
Tesla.prototype.setChargeMode = function (vehicleId, chargeModeType) {
  ...
  return new Promise(function (resolve, reject) {
    if (chargeModeTypeList.indexOf(chargeModeType.toLowerCase()) < 0) return reject('mode_invalid')
    checkActiveGrant(self).then(function () {
      postVehicleCommand(self.grant.access_token, vehicleId, 'command/charge_' + chargeModeType.toLowerCase(), null, function (error, state) {
        if (error) reject(error)
        else resolve(state)
      })
    })
...
Tesla.prototype.wakeUp = function (vehicleId) {
  ...
  return new Promise(function (resolve, reject) {
    checkActiveGrant(self)
    .then(function () {
      postVehicleCommand(self.grant.access_token, vehicleId, 'wake_up', null, function (error, state) {
        if (error) reject(error)
        else resolve(state)
      })
    })
