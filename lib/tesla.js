...
Tesla.prototype.setChargeMode = function (vehicleId, chargeModeType) {
  ...
  return new Promise(function (resolve, reject) {
    if (chargeModeTypeList.indexOf(chargeModeType) < 0) return reject('mode_invalid')
    checkActiveGrant(self).then(function () {
      postVehicleCommand(self.grant.access_token, vehicleId, 'command/charge_' + chargeLimitType, null, function (error, state) {
        if (error) reject(error)
        else resolve(state)
      })
    })
