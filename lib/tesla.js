...
Tesla.prototype.controlAutoConditioning = function (vehicleId, state) {
  ...
  return checkActiveGrant(self).then(function () {
    return postVehicleCommand(self.grant.access_token, vehicleId, `command/auto_conditioning_${state ? 'start' : 'stop'}`, null)
  })
}
...
Tesla.prototype.controlChargePort = function (vehicleId, portstate) {
  ...
  return checkActiveGrant(self).then(function () {
    return postVehicleCommand(self.grant.access_token, vehicleId, 'command/charge_port_door_open', null)
  })
}
...
Tesla.prototype.controlCharging = function (vehicleId, state) {
  ...
  if (typeof state !== 'boolean') return Promise.reject('state_invalid')
  return checkActiveGrant(self).then(function () {
    return postVehicleCommand(self.grant.access_token, vehicleId, `command/charge_${state ? 'start' : 'stop'}`, null)
  })
}
...
Tesla.prototype.controlDoorLock = function (vehicleId, state) {
  ...
  if (typeof state !== 'boolean') return Promise.reject('state_invalid')
  return checkActiveGrant(self).then(function () {
    return postVehicleCommand(self.grant.access_token, vehicleId, `command/door_${state ? 'lock' : 'unlock'}`, null)
  })
}
...
Tesla.prototype.controlPanoRoof = function (vehicleId, roofstate) {
  ...
  return checkActiveGrant(self).then(function () {
    return postVehicleCommand(self.grant.access_token, vehicleId, 'command/sun_roof_control', {state: roofstate})
  })
}
...
Tesla.prototype.controlPanoRoofPercentage = function (vehicleId, limit) {
  ...
  return checkActiveGrant(self).then(function () {
    return postVehicleCommand(self.grant.access_token, vehicleId, 'command/sun_roof_control', {state: 'move', percent: limit})
  })
}
...
Tesla.prototype.controlValetMode = function (vehicleId, state, pin) {
  ...
  return checkActiveGrant(self).then(function () {
    return postVehicleCommand(self.grant.access_token, vehicleId, 'command/set_valet_mode', data)
  })
}
...
Tesla.prototype.flashLights = function (vehicleId) {
  var self = this
  return checkActiveGrant(self).then(function () {
    return postVehicleCommand(self.grant.access_token, vehicleId, 'command/flash_lights', null)
  })
}
...
Tesla.prototype.getChargeState = function (vehicleId) {
  var self = this
  return checkActiveGrant(self).then(function () {
    return getVehicleCommand(self.grant.access_token, vehicleId, 'data_request/charge_state')
  })
}
...
Tesla.prototype.getClimateState = function (vehicleId) {
  var self = this
  return checkActiveGrant(self).then(function () {
    return getVehicleCommand(self.grant.access_token, vehicleId, 'data_request/climate_state')
  })
}
...
Tesla.prototype.getDriveState = function (vehicleId) {
  var self = this
  return checkActiveGrant(self).then(function () {
    return getVehicleCommand(self.grant.access_token, vehicleId, 'data_request/drive_state')
  })
}
...
Tesla.prototype.getGuiSettings = function (vehicleId) {
  var self = this
  return checkActiveGrant(self).then(function () {
    return getVehicleCommand(self.grant.access_token, vehicleId, 'data_request/gui_settings')
  })
}
...
Tesla.prototype.getMobileAccess = function (vehicleId) {
  var self = this
  return checkActiveGrant(self).then(function () {
    return getVehicleCommand(self.grant.access_token, vehicleId, 'mobile_enabled')
  })
}
...
Tesla.prototype.getVehicleState = function (vehicleId) {
  var self = this
  return checkActiveGrant(self).then(function () {
    return getVehicleCommand(self.grant.access_token, vehicleId, 'data_request/vehicle_state')
  })
}
...
Tesla.prototype.honkHorn = function (vehicleId) {
  var self = this
  return checkActiveGrant(self).then(function () {
    return postVehicleCommand(self.grant.access_token, vehicleId, 'command/honk_horn', null)
  })
}
...
Tesla.prototype.remoteStart = function (vehicleId) {
  ...
  return checkActiveGrant(self).then(function () {
    return postVehicleCommand(self.grant.access_token, vehicleId, 'command/remote_start_drive', {password: self.password})
  })
}
...
Tesla.prototype.resetValetPin = function (vehicleId) {
  var self = this
  return checkActiveGrant(self).then(function () {
    return postVehicleCommand(self.grant.access_token, vehicleId, 'command/reset_valet_pin', null)
  })
}
...
Tesla.prototype.setAutoConditioningTemperatures = function (vehicleId, driver, passenger) {
  ...
  return checkActiveGrant(self).then(function () {
    return postVehicleCommand(self.grant.access_token, vehicleId, 'command/set_temps', values)
  })
}
...
Tesla.prototype.setChargeLimit = function (vehicleId, limit) {
  ...
  return checkActiveGrant(self).then(function () {
    return postVehicleCommand(self.grant.access_token, vehicleId, 'command/set_charge_limit', {percent: limit})
  })
}
...
Tesla.prototype.setChargeMode = function (vehicleId, chargeModeType) {
  ...
  return checkActiveGrant(self).then(function () {
    return postVehicleCommand(self.grant.access_token, vehicleId, 'command/charge_' + chargeModeType.toLowerCase(), null)
  })
}
...
Tesla.prototype.wakeUp = function (vehicleId) {
  var self = this
  return checkActiveGrant(self).then(function () {
    return postVehicleCommand(self.grant.access_token, vehicleId, 'wake_up', null)
  })
}

// Validates token if token seems invalid
function checkActiveGrant (self) {
  if (!grantExpired(self.grant)) return Promise.resolve()
  return self.login()
}
...
function getVehicleCommand (token, vehicleId, command) {
  return new Promise(function (resolve, reject) {
    ...
    if (!vehicleId) return reject('no_vehicleId')
    ...
    http.json(options).then(function (result) {
      if (!result.response) return reject(result.error || 'api_error')
...
function postVehicleCommand (token, vehicleId, command, body) {
  return new Promise(function (resolve, reject) {
    ...
    http.post(options).then(function (result) {
      if (!result.response) return reject('error')
      if (result.response.statusCode !== 200) return reject(result.response)
